...
class Settings extends Component
{
    ...
    public function mount()
    {
        ...
        foreach (['logo','logo_admin','logo_footer','favicon','metaimg','imgheader','watermark'] as $field) {
            if (!empty($config->$field)) {
                $this->$field = Storage::url($config->$field);
            } else {
    ...
    {      
        try {
            ...
            $this->logo = $snapshots['logo'];
            $this->logo_admin = $snapshots['logo_admin'];
            $this->logo_footer = $snapshots['logo_footer'];
            $this->favicon = $snapshots['favicon'];
            $this->watermark = $snapshots['watermark'];
            $this->metaimg = $snapshots['metaimg'];
            $this->imgheader = $snapshots['imgheader'];
    ...
    public function getQrCodeSvgProperty()
    {
        return QrCode::size(240)
            ...
            ->generate($this->configData['domain'] ?? config('app.url', 'https://informaticalivre.com.br'));
    }
